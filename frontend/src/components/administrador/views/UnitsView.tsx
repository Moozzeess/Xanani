...
export default function UnitsView() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [resUnidades, resHardware, resConductores] = await Promise.all([
        api.get('/units', { headers }),
        api.get('/hardware', { headers }),
        api.get('/users/conductores', { headers }).catch(() => ({ data: { data: { conductores: [] } } }))
      ]);
      ...
      const conductorsData = resConductores.data?.data?.conductores || [];
  ...
  const handleSubmitUnidad = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = isEditing ? `/units/${currentUnitId}` : '/units';
  ...
  return (
    <div id="view-units" className="space-y-6 relative animate-in fade-in duration-500">
              ...
              {cargando ? (
                ...
                unidadesFiltradas.map((u, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-bold text-slate-700">{u.capacidad} pax</td>
                    <td className="px-6 py-4">
